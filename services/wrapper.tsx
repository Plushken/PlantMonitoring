...
const Wrapper = (AppComponent: React.ComponentType<any>, config: Config, onHide?: () => void) => {
  const { api } = config;
  ...
  const WrappedComponent = () => {
    ...
    if (!isConnected) {
      return (
        <ScreenWrapper>
          <Text style={{ color: '#fff', fontSize: 32, textAlign: 'center' }}>{'NO INTERNET'}</Text>
        </ScreenWrapper>
      );
    }
    ...
    if (isLoading) {
      return (
        <ScreenWrapper>
          <Text style={{ color: '#fff', fontSize: 32, textAlign: 'center' }}>{'Loading...'}</Text>
